...
export default function PurchaseLedger() {
  ...
  useEffect(() => {
    ...
    try {
      ...
      if (storedInvoices) {
        ...
        processedInvoices.sort((a, b) => new Date(b.invoiceDate).getTime() - new Date(a.invoiceDate).getTime());
        setInvoices(processedInvoices);
        setNoDataFound(processedInvoices.length === 0);
      } else {
        setNoDataFound(true);
      }
    } catch (error) {
    ...
    try {
      ...
      if (storedPayments) {
        ...
        parsedPayments.sort((a, b) => new Date(b.date).getTime() - new Date(a.date).getTime());
  ...
  const deleteInvoice = (invoiceNo: string) => {
    ...
    localStorage.setItem('feedPurchaseInvoices', JSON.stringify(updatedInvoices));
    toast({
